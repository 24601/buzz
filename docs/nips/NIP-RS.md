...
`C` yields nothing at the start of a load, and yields nothing for the whole of a load whose entire history at this kind is a single event: one delivery of one event bounds the cap below by one, and no delivery can be smaller than that. The procedure therefore also fixes a floor, `L = 2` — a relay will deliver two events for a query whose matching set contains at least two. This is a floor on relay behaviour and MUST NOT be derived from relay-advertised discovery, because an advertised maximum is not necessarily the limit a relay enforces. It is stated at the smallest value that admits a second event, because a larger floor is a stronger claim about relays that buys nothing further: a relay that will deliver only one event per query cannot express `limit` semantics and cannot serve a user who has two coordinates at all, whereas any floor above two would begin excluding relays this procedure does not need to exclude.

1. Every query MUST carry the same explicit `limit` and MUST NOT constrain tags. `C` is only meaningful across queries that differ solely in their time bounds.
2. From each delivered event, collect the read-state coordinates — those whose `d` tag has the form `read-state:<slot-id>` — deduplicating by `d` tag value and retaining the entry with the highest `created_at`. Ignore the other events, but count them: they are part of what the cap returned.
3. Let `T` be the lowest `created_at` across **all** events delivered so far, not only the read-state ones. The cursor therefore advances on every non-empty page, including one that yielded no coordinate.
...
7. The load is **complete** when a continuation delivers no events at all, every preceding second having been discharged by step 5. Because the filter constrains nothing the relay applies after its cap, an empty delivery is an empty result — a cap that returns nothing is not a cap.
8. A load that failed on any relay the client publishes to is potentially incomplete (see Read-Before-Write).

This procedure assumes three things of a relay, and states them because the rest of the section rests on them. A relay serving a fixed filter returns a deterministic prefix of a total order over the matching set; against one that caps by choosing an arbitrary subset instead, no bounded enumeration on the standard filter surface establishes completeness, and repeating a query cannot help, because the same filter with the same bounds is the same request. A relay's cap does not shrink between queries of the same shape within a single load; a relay that returns fewer events for a later query than an earlier one — for reasons other than having fewer to return — can make `C` overstate what it will still deliver. And a relay honours the floor `L`.
...
A context's frontier entry and ALL of its `ov_*` sibling entries MUST travel in the same event. Because all `ov_*` entries live in the primary coordinate (see `d` Tag), this means the frontier entry of any context that carries override state MUST also live in the primary: a client that splits frontier entries into additional coordinates MUST NOT move the frontier entry of an override-bearing context out of the primary, and MUST NOT place `ov_*` entries anywhere else.
