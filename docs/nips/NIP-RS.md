...
5. Second `T` is exhausted only if that pinned query delivered fewer than `C` events. If it delivered `C` or more, the cap may have bound inside the second, no finer cursor exists on the standard filter surface, and the load MUST be treated as **potentially incomplete**.
...
7. The load is **complete** when a continuation delivers no events at all. Because the filter constrains nothing the relay applies after its cap, an empty delivery is an empty result — a cap that returns nothing is not a cap.
8. A load that failed on any relay the client publishes to is potentially incomplete (see Read-Before-Write).

This procedure assumes two things of a relay, and states them because the rest of the section rests on them. A relay serving a fixed filter returns a deterministic prefix of a total order over the matching set; against one that caps by choosing an arbitrary subset instead, no bounded enumeration on the standard filter surface establishes completeness, and repeating a query cannot help, because the same filter with the same bounds is the same request. And a relay's cap does not shrink between queries of the same shape within a single load; a relay that returns fewer events for a later query than an earlier one — for reasons other than having fewer to return — can make `C` overstate what it will still deliver.

The comparison in step 5 fails safe: a pinned window is reported potentially incomplete unless the relay has already shown it will deliver more at once, so an inconclusive result is never mistaken for an exhaustive one. Two consequences follow, and neither can produce a false *complete*. A plateau of more events at a single `created_at` than the relay will deliver for a window pinned to that second is unenumerable, because this NIP defines no finer cursor. And a client whose entire history at this kind lies within one second has no larger delivery to compare against, so it cannot discharge step 5 at all; because an ordinary frontier publish is not a canonical publication and so does not itself require a complete load, that state clears as soon as the client publishes in a later second.
