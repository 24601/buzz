...
Removing `since` does not by itself make the result complete. Relays MAY cap the number of events returned for a historical query, MAY cap below the client's requested `limit`, MAY apply tag constraints only after that cap, and emit end-of-stored-events after the capped query — so **a single query proves nothing.** End-of-stored-events marks the end of the capped result, not the end of the matching set, and a short result does not establish that no further coordinates exist. Caps typically retain the newest events and drop the oldest, which are precisely the rotation-predecessor and orphaned coordinates whose tombstone floors this layer depends on. A silently truncated load that omits the sole carrier of a floor merges a stale live register unopposed and reports a manually-unread context as read, permanently.

The load MUST therefore be completed by continuation on a strictly decreasing cursor. A client MUST NOT test completeness by comparing the number of events returned against the `limit` it requested: the effective cap is the relay's, a relay MAY cap below the requested value, and a relay's advertised maximum limit is not necessarily the limit it enforces — so no comparison against the requested `limit` is a valid truncation test.

1. The filter MUST carry an explicit `limit`.
2. Collect every returned event, deduplicating by coordinate: for each `d` tag value, retain the entry with the highest `created_at`.
3. Let `T` be the lowest `created_at` collected so far. Before advancing past `T`, the client MUST re-query the single-second window `{"since": T, "until": T}` and merge the result, repeating until that window yields no coordinate the client has not already collected. This step is what recovers coordinates a cap dropped: a broad page can be filled to the cap by same-kind events under unrelated `d` tag families that the client's tag constraint then discards, and pinning both bounds to one second removes every event outside that second from contention for the cap.
4. Continue with `"until": T - 1`. Because a bare `until` is inclusive, decrementing guarantees each continuation covers a strictly older band, so the loop makes progress regardless of how the relay caps.
5. The load is **complete** when a continuation returns no events at all.
6. A load that failed on any relay the client publishes to is potentially incomplete (see Read-Before-Write).

Two properties of relay behaviour make the pinned window in step 3 load-bearing, and both MUST be assumed. A relay MAY apply tag constraints *after* its result cap, so a page smaller than the cap does not mean the matching set was exhausted. And a relay MAY cap a page below what the client asked for without signalling that it did. Together these mean **a short page proves nothing**; only an empty one does, which is why step 5 terminates on empty rather than on short.

One residual is not closed by this procedure: if more `read-state` coordinates share a single one-second `created_at` value than the relay will return for a window pinned to that second, the surplus cannot be enumerated on the standard filter surface, and the client cannot detect that this happened. This NIP defines no finer cursor. Reaching it requires a client to publish more coordinates within one second than a relay's page cap, which no conforming publication pattern produces.
